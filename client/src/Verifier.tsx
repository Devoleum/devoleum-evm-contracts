...
const Verifier = (props) => {
  ...
  const blockchainNameAttr = props.blockchainName === 'Polygon Matic' ? 'polygon_matic_notarization' : 'test_eth_notarization';

  const getDevoleumStep = async () => {
    let step: IStep = await getData(`${process.env.API_BASE_URL}/api/steps/${itemId}`);
    const jsonContent = await getData(step.uri);
    setHash(await calcHash(
